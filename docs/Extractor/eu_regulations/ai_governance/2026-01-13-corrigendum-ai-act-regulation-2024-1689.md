---
title: "Corrigendum to Regulation (EU) 2024/1689 on harmonised rules concerning artificial intelligence (AI Act)"
source_url: https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32024R1689
date: 2026-01-13
category: ai_governance
confidence: 高
---

## L1 — Rule Signal
- **rule_type**: amendment
- **issuing_body**: European Union
- **document_id**: CELEX:32024R1689
- **status**: final
- **document_type**: regulation
- **binding_force**: directly_applicable
- **effective_date**: 2025-12-19
- **transposition_deadline**: N/A

## L2 — Responsibility Structure
- **affected_roles**:
  - AI system providers
  - AI system deployers
  - Online platform operators
  - National competent authorities
- **shift_type**: clarified
- **shift_summary**: 明確排除線上空間適用範圍，修正原條文錯誤描述

## L3 — Risk Domains
- AI 治理與合規
- 線上平台監管
- 產品安全與責任

## L4 — Obligation & Evidence
- **new_obligations**:
  - 線上空間（online spaces）不適用本法規範圍，因其並非實體空間
- **evidence_requirements**:
  - 實體空間與線上空間之分類證明文件
  - AI 系統部署環境說明文件
- **enforcement_signal**: mandatory

## Notes
此為 AI Act（Regulation 2024/1689）第 2 次勘誤文件（R(02)），修正第 6 頁 recital 19 的關鍵錯誤。原文誤將「線上空間納入適用範圍」，勘誤後明確「線上空間不在適用範圍內，因其並非實體空間」。此修正具實質影響，會改變 AI 系統在線上環境部署時的合規義務判定。

原 AI Act 涉及航空安全、機械、鐵路、海事、車輛等多項 EU 法規的修訂，以及智慧財產權執行指令。
