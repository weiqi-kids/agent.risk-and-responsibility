---
title: "Corrigendum to Regulation (EU) No 1233/2011 on the application of certain guidelines in the field of officially supported export credits"
source_url: "https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32011R1233"
date: 2025-01-30
category: institutional_administration
confidence: 高
---

## L1 — Rule Signal

- **rule_type**: amendment
- **issuing_body**: European Parliament and Council
- **document_id**: 32011R1233R(01)
- **status**: final
- **document_type**: regulation
- **binding_force**: directly_applicable
- **effective_date**: 2026-02-05

## L2 — Responsibility Structure

- **affected_roles**: export credit agencies, member state authorities, Irish language translators
- **shift_type**: clarified
- **shift_summary**: 修正愛爾蘭語版本中「officially supported export credits」的語法形式，將過去分詞改為現在分詞

## L3 — Risk Domains

- institutional_administration
- financial_compliance

## L4 — Obligation & Evidence

- **new_obligations**:
  - 使用修正後的愛爾蘭語術語
- **evidence_requirements**:
  - 確保官方文件引用正確語法形式
- **enforcement_signal**: mandatory

## Notes

- 本勘誤修正愛爾蘭語版本共 26 處術語：
  - 原文：「arna dtacú」（過去分詞）
  - 修正：「a dtacaítear leo」（現在分詞）
- 影響範圍：
  - 文件標題與前言
  - Article 1
  - Annex I（報告要求）
  - Annex II（Arrangement 條款）
  - Annexes III-V（核能、航空、再生能源部門協議）
- 屬語法修正，不影響實質義務
- WebFetch 重試成功（使用 `/EN/TXT/` 替代 `/AUTO/`）
