---
title: "Corrigendum to Regulation (EU) 2024/1349 on funding for migration challenges in border regions"
source_url: https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32024R1349
date: 2025-02-04
category: institutional_administration
confidence: 高
---

## L1 — Rule Signal

- **rule_type**: amendment
- **issuing_body**: European Parliament and Council
- **document_id**: 32024R1349R(02)
- **status**: final
- **document_type**: regulation
- **binding_force**: directly_applicable
- **effective_date**: 2026-02-06

## L2 — Responsibility Structure

- **affected_roles**: border region authorities, migration management agencies, German language translators
- **shift_type**: clarified
- **shift_summary**: 修正德語版本標題中的術語，將「Rückführungsverfahren」改為「Rückkehrverfahren」

## L3 — Risk Domains

- institutional_administration

## L4 — Obligation & Evidence

- **new_obligations**:
  - 使用修正後的德語術語
- **evidence_requirements**:
  - 確保官方文件引用正確術語
- **enforcement_signal**: mandatory

## Notes

- 本勘誤僅修正德語版本標題中的術語：
  - 原文：「Rückführungsverfahren」
  - 修正：「Rückkehrverfahren」
  - 兩者均指「返回程序」，屬術語統一修正
- 原 Regulation 2024/1349 規範邊境地區移民挑戰的資金使用
- 影響範圍：僅標題頁
- Reference: OJ L 2026/90093
- WebFetch 重試成功（使用 `/EN/TXT/` 替代 `/AUTO/`）
